...
class Exp_Classification_Save_FullTrainOpt_WL(Exp_Basic):
    def __init__(self, args):
        super(Exp_Classification_Save_FullTrainOpt_WL, self).__init__(args)
    ...
    def _build_model(self):
        ...
        print("🔧 Building model and loading metadata...", flush=True)
    ...
    def train(self, setting):
        ...
        #neg = (y_train == 0).sum()
        #pos = (y_train == 1).sum()
        #class_weights = torch.tensor([1.0, neg / pos], dtype=torch.float).to(self.device)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, padding_mask) in enumerate(train_loader):
                ...
                #    print(
                #        f"[Epoch {epoch+1:02d}/{self.args.train_epochs}] "
                #        f"Iter {i+1:04d}/{train_steps} | "
                #        f"Loss={loss.item():.4f} | "
                #        f"Speed={speed:.4f}s/iter | "
                #        f"ETA={left_time:.1f}s",
                #        flush=True
                #    )
                #    iter_count = 0
                #    time_now = time.time()
            ...
            #print(
             ...
             #print(
              #  f"\n📌 Epoch {epoch+1:02d}/{self.args.train_epochs} Summary | "
               # f"Steps={train_steps} | "
                #f"Train Loss={train_loss:.4f} | "
                #f"Val Loss={vali_loss:.4f} | Val F1={val_f1:.4f} | "
                #f"Test Loss={test_loss:.4f} | Test F1={test_f1:.4f}\n",
                #flush=True
            #)
            epoch_time_total = time.time() - epoch_time

            print(
                f"[Epoch {epoch+1:02d}/{self.args.train_epochs}], "
                f"Time={epoch_time_total:.2f}s, "
                f"Train Loss={train_loss:.4f}, "
                f"Val Loss={vali_loss:.4f}, Val F1={val_f1:.4f}, "
                f"Test Loss={test_loss:.4f}, Test F1={test_f1:.4f}",
                flush=True
        ...
        pd.DataFrame(loss_history).to_csv(
            os.path.join(save_path, "loss_history.csv"),
            index=False
        )

        print(f"✅ Saved loss history to {os.path.join(save_path, 'loss_history.csv')}")


        self.model.load_state_dict(torch.load(best_model_path))
        # ✅ ADD THIS — save VAL probabilities for NAS scoring
        # ============================================================
        # Save TRAIN + VAL probabilities from best checkpoint
        # ============================================================
        ...
        def collect_probs(loader):
            ...
            with torch.no_grad():
                for batch_x, label, padding_mask in loader:
                    ...
                    trues_all.extend(label.flatten().cpu().numpy())

            return np.array(probs_all), np.array(trues_all)


        train_probs_all, train_trues_all = collect_probs(train_loader)
        val_probs_all, val_trues_all = collect_probs(vali_loader)

        np.save(os.path.join(save_path, 'train_prob.npy'), train_probs_all)
        np.save(os.path.join(save_path, 'train_true.npy'), train_trues_all)

        np.save(os.path.join(save_path, 'val_prob.npy'), val_probs_all)
        np.save(os.path.join(save_path, 'val_true.npy'), val_trues_all)

        print("✅ Saved TRAIN and VAL probabilities for gap analysis")
        return self.model        
